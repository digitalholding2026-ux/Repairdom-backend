...
export class EmailService {
  ...
  constructor(private readonly config: ConfigService) {
    this.from = this.config.get<string>('SMTP_FROM') ?? 'Relio <noreply@repairdom.app>';
  ...
  async sendVerificationEmail(to: string, verificationLink: string): Promise<void> {
    ...
    await this.transporter.sendMail({
      ...
      html: [
        ...
        '<p style="margin:24px 0"><a href="' + verificationLink + '" style="background:#007bff;color:#ffffff;text-decoration:none;padding:12px 22px;border-radius:8px;font-weight:600">Je vérifie mon adresse email</a></p>',
